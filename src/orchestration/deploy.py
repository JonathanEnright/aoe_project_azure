...
class DeploymentConfig:
    ...
    databricks_host: str = field(
        default_factory=lambda: os.environ.get("DATABRICKS_HOST_URL")
    )
    databricks_token: str = field(
        default_factory=lambda: os.environ.get("DATABRICKS_PAT")
    )
...
class DatabricksDeployer:
    ...
    def upload_wheel(self, wheel_path: Path) -> str:
        ...
        logger.info(
            f"Uploading wheel '{wheel_path.name}' to Databricks workspace at '{workspace_path}'..."
        )
    ...
    def create_or_update_job(
        ...
        job_tasks = [
            Task(
                ...
                python_wheel_task=PythonWheelTask(
                    package_name=package_name, entry_point=t["entry_point"]
                ),
                libraries=[Library(whl=wheel_path)],
                depends_on=[
                    TaskDependency(task_key=d) for d in t.get("depends_on", [])
                ],
        ...
        job_settings = JobSettings(
            ...
            email_notifications=JobEmailNotifications(
                on_start=notifications.get("on_start", [])
            ),
        ...
        for job in existing_jobs:
            if job.settings.name == self.config.job_name:
                job_id = job.job_id
                logger.info(
                    f"Existing job '{self.config.job_name}' found (Job ID: {job_id}). Updating it."
                )
                break
        ...
        if job_id:
            ...
            logger.info(
                f"Job '{self.config.job_name}' (Job ID: {job_id}) updated successfully."
            )
        else:
            # Create new job
            ...
            logger.info(
                f"New job '{self.config.job_name}' created successfully (Job ID: {job_id})."
            )
    ...
    def monitor_run(self, run_id: int, job_id: int):
        logger.info(
            f"Monitoring job run '{run_id}'. Visit: {self.config.databricks_host}/jobs/{job_id}/runs/{run_id}"
        )
        ...
        try:
            while True:
                ...
                if state in [
                    ...
                    logger.info(
                        f"[{elapsed:.0f}s elapsed] Job run status: {state.value}. Continuing monitoring..."
                    )
                    time.sleep(self.config.check_interval_secs)
                elif result_state == RunResultState.SUCCESS:
                    logger.info(
                        f"✅ Job run completed successfully in {elapsed:.0f} seconds."
                    )
                    break
                else:
                    logger.error(
                        f"❌ Job run ended with state: {state.value}, result: {result_state.value}"
                    )
                    break

        except KeyboardInterrupt:
            logger.warning(
                "Keyboard interrupt received. Attempting to gracefully cancel the Databricks job..."
            )
...
def deploy_pipeline(
    ...
    job_id = deployer.create_or_update_job(
        pipeline_tasks, wheel_workspace_path, notifications or {}
    )
    ...
    if run_now:
        ...
        logger.info(
            f"Pipeline '{config.job_name}' deployed successfully and scheduled."
        )
