...
def get_filters(transformed_df: pd.DataFrame, prefix):
    ...
    st.session_state[f"{prefix}_opponent_civ_query"] = [
        el for el in opponent_civ_select
    ]
    st.session_state[f"{prefix}_map_query"] = [el for el in map_select]
    st.session_state[f"{prefix}_match_elo_bucket_query"] = [
        el for el in match_elo_bucket_select
    ]
